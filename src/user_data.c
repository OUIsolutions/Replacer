...
UserData * extract_user_informations(){
    ...
    *user = (UserData){0};
    ...
    user->action = cli_interface.ask_option( &cli_interface,"type the action","search | replace | restore");
    if(user->action == RESTORE){
        while(true){
            char *not_formated_backup_file = cli_interface.ask_string(&cli_interface,"type the name of your backup",CLI_TRIM);
            ...
            if(dtw_entity_type(backup_modifed->rendered_text) != DTW_FILE_TYPE){
                ...
                cli_interface.warning(&cli_interface,already_exist_mensage->rendered_text);
    ...
    if(user->action == SEARCH){
        user->first_token = cli_interface.ask_string(&cli_interface,"type the element to search",CLI_NOT_TRIM);
    }


    if(user->action == REPLACE){
        user->first_token = cli_interface.ask_string(&cli_interface,"type the element that will be replaced",CLI_NOT_TRIM);
        user->second_token = cli_interface.ask_string(&cli_interface,"type the element to replace",CLI_NOT_TRIM);
        user->second_token_size = (int)strlen(user->second_token);

        bool store_backup = cli_interface.ask_option(&cli_interface,"store backup?","no | yes");
        if(store_backup) {
    ...
    user->case_sensitive = cli_interface.ask_option( &cli_interface,"case sensitive?","no | yes");
    ...
    while(true){
        if(user->action == REPLACE){
            user->source = cli_interface.ask_string(&cli_interface,"type the folder/file to replace (. for current folder)",CLI_TRIM);
        }
        else{
            user->source = cli_interface.ask_string(&cli_interface,"type the folder/file to search (. for current folder)",CLI_TRIM);
        }
        ...
        if(user->type_of_source == DTW_NOT_FOUND){
            ...
            cli_interface.warning(&cli_interface,s->rendered_text);
            free(user->source);
            stack.free(s);
            continue;
        }
